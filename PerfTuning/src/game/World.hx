...
class World 
{
	...
	var count:Int = 0;
	var worldScale:Float = 10.0;

	public function new() 
	{
		var edge = 2000;
		var area = edge * edge * worldScale;
		var edge = Std.int(Math.sqrt(area));
		var numAI = Std.int(10 * worldScale);
		var numFood = Std.int(50 * worldScale);
		...
		for (i in 0...numAI) createAi();
		for (i in 0...numFood) createFood();
	}
